...
# automatically retreived and written to ./azureConfig/local_secrets.json.
...
function Deploy-Infrastructure {
    ...
    process {
        ...
        # azureConfig/otel-local-config.yaml
        ...
        # Populating azureConfig/otel-local-config.yaml
        $config = $(Get-Content ../azureConfig/otel-local-config.yaml | Convertfrom-Yaml)
        $config.exporters.azuremonitor.instrumentation_key = $instrumentationKey

        Write-Output 'Saving ./azureConfig/otel-local-config.yaml'
        $config | ConvertTo-Yaml | Set-Content ../azureConfig/otel-local-config.yaml
    }
...
if ($cloud.IsPresent) {
    ...
    # If you don't find the ./azureConfig/local_secrets.json run the setup.ps1 in deploy folder
    if ($config.exporters.azuremonitor.instrumentation_key -eq "") {
        Write-Output "./azureConfig/local_secrets.json not found running setup"
        Deploy-Infrastructure -rgName $rgName -location $location
    }
